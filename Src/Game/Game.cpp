...
void Game::buildHeldBlockMesh(Block block) {
	...
	mesh.loadVAOs();
	m_heldBlockMesh = std::move(mesh);
}

void Game::renderHeldBlock() {
	std::optional<Block> picked = m_player.getPickedBlock();
	if (!picked.has_value())
		return;
	...
	const Shader& shader = m_defaultRenderer.getShader();
	...
	m_defaultRenderer.render(m_heldBlockMesh);
