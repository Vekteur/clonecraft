...
void main()
{
	...
	float distanceFactor = 10.f / fs_in.distanceFromPlayer; // TO IMPROVE
	...
	vec4 reflectColor = underwater ? refractColor : vec4(skyColor, 1.f);
	// Face visible from top and at the sea level
	if (!simple && !underwater && abs(fs_in.norm.y - 1.f) < epsilon && fs_in.pos.y == 64) {
	...
	float refractiveFactor = clamp(dot(viewVector, fs_in.norm), 0.f, 1.f);
